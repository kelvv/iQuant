...
国信 iQuant 官方支持两种外部程序下单，按 `.env` 的 `ORDER_MODE` 选：

| 模式 | 机制 | 状态 | 前提 |
|------|------|------|------|
| `file` | 写 txt → 平台「文件单」策略监听 → 下单（**国信官方文档 5.3.2.12**） | 待开权限 | 管理端开「允许使用函数交易下单」+ 客户端启动文件单策略 |
| `miniqmt` | `xttrader` Python 接口连客户端 | connect=-1 | miniQMT 交易接口未开放 |
| `dry` | 只判定+日志，不下单 | ✅ 可用 | 无 |

**两条路都卡在同一个根因**：国信「程序交易/函数交易下单」权限是**管理端**（券商侧）控制的，
当前账户没开通（交易终端里没有文件单按钮 = 权限没开，文档 3134 行）。
**需联系国信对接人开通「允许使用函数交易下单」权限**（模拟盘一般很简单）。

权限开通后：
- 走 `file`：客户端「文件单」窗口新建策略 → 绑账号组 + 文件路径(=`FILE_ORDER_PATH`) → 启动
- 走 `miniqmt`：客户端启用极速交易 → `trader.py conn` 应返回账户资产

### 文件单 txt 格式（文档原文）
```
下单类型,通道号,报价方式,下单代码,下单总量
23,1,8.9,600004,200      # 23=买入 1=通道 8.9=指定价 600004=代码 200=量
```
